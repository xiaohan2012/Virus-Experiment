...
if __name__ == "__main__":
    ...
    from ve.config import data237_raw_complex as c_root, data237_fps808015_root as fp_root, data237_root as root
    from ve.util.load_pdb import complex_ids
    
    import sys
    """
    single | double
    15bits.fp | atb.fp |  atg.fp
    """
    try:
        fp_type1, fp_type2 = sys.argv[1:3]
    except IndexError,ValueError:
        print "invalid arguments"
        sys.exit(0)
    
    mat_dir = os.path.join(root,"complex_sim_mat")

    if not os.path.exists(mat_dir): os.makedirs(mat_dir)

    mat_path = os.path.join(mat_dir, "%s_%s.mat" %(fp_type1,fp_type2.split(".")[0]))

    sim_mat = defaultdict(dict) if not os.path.exists(mat_path) else load(open(mat_path))
        
    c_ids = complex_ids(c_root)
    for c1_id in c_ids:
        for c2_id in c_ids:
            print "considering",c1_id,c2_id
            if (sim_mat.has_key(c1_id) and sim_mat[c1_id].has_key(c2_id)):
                print "computed already"
                continue
            
            c_fp = os.path.join(c_root, c1_id + ".pdb")
            fp_fp = os.path.join(fp_root, fp_type1, c1_id, fp_type2)
            
            try:
                #some part might not be ready
                fp1 = FPWithComplex(c_fp, fp_fp)
            except IOError:
                print "not ready yet"
                continue

            c_fp = os.path.join(c_root, c2_id + ".pdb")
            fp_fp = os.path.join(fp_root, fp_type1, c2_id, fp_type2)

            try:
                fp2 = FPWithComplex(c_fp, fp_fp)
            except IOError:
                print "not ready yet"
                continue
            
            sim_vals = similarity_between(fp1,fp2)
            
            print sim_vals

            sim_mat[c1_id][c2_id] = sim_vals
            sim_mat[c2_id][c1_id] = sim_vals

            print "updating pickle"
            dump(sim_mat,open(mat_path,"w"))
